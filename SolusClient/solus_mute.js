...
function showMuteList() {
    ChatLib.chat("§3§m---------------------------------------------");
    ChatLib.chat("§bMutes Locaux ("+Object.keys(S.data.muted).length+"):");
    Object.keys(S.data.muted).forEach(x => ChatLib.chat(" §7- §e" + S.data.muted[x].name));
...
register("command", (...args) => {
    if (!args || args.length === 0) {
        ChatLib.chat("§cUsage: /smute <pseudo> [temps: 10m, 1h]");
        ChatLib.chat("§cUsage: /smute list");
        return;
    }
